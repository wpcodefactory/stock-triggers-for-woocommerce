	...
	function get_settings() {
		...
		foreach ( $require_product as $id => $values ) {
			foreach ( $values as $value ) {
				...
				$require_product_options[ $id ][ esc_attr( $value ) ] = (
					is_object( $product ) ?
					esc_html( wp_strip_all_tags( $product->get_formatted_name() ) ) :
					sprintf(
						/* Translators: %d: Product ID. */
						esc_html__( 'Product #%d', 'stock-triggers-for-woocommerce' ),
						$value
					)
				);
			}
		...
		foreach ( $sections as $id => $data ) {
			$settings = array_merge( $settings, array(
				...
				array(
					...
					'desc'              =>
						__( 'One action per line.', 'stock-triggers-for-woocommerce' ) . ' ' .
						sprintf(
							/* Translators: %s: Trigger example. */
							__( 'E.g.: %s', 'stock-triggers-for-woocommerce' ),
				...
				array(
					...
					'desc_tip'          => sprintf(
						...
						__( 'Removes standard WooCommerce "%s" triggers.', 'stock-triggers-for-woocommerce' ),
						$data['desc']
					),
				...
				array(
					...
					'desc_tip'          =>
						__( 'Select products which have to be in the order for stock action to be triggered.', 'stock-triggers-for-woocommerce' ) . ' ' .
